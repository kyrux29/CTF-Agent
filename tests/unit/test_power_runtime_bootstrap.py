...
def test_power_bootstrap_completes_copied_example_placeholders(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """A standard copied .env.example becomes a working Power configuration."""

    module = _bootstrap_module()
    destination = tmp_path / ".env"
    destination.write_text(
        (module.ROOT / ".env.example").read_text(encoding="utf-8"), encoding="utf-8"
    )
    monkeypatch.setattr(module, "DEFAULT_ENV_PATH", destination)
    monkeypatch.setattr(module, "_docker_socket_gid", lambda: "1000")
