...
8. Skill bổ sung của Power chỉ đọc từ `services/pi-runner/reviewed-skill-packs/`
   đã được manifest và đóng vào runner image; không auto-discover skill trong
   challenge hoặc filesystem runtime.
...
Turn authority / lease giữ như `services/pi-runner/src/tools.ts` hiện có. Power racer = session role `exploit_builder` (đủ tool) hoặc role hẹp hơn cho AutoPrompter (cấm `ctf_flag_submit`). Khi `stdout` hoặc `stderr` quan sát được khớp format flag của run, control plane giữ **chỉ session racer đã báo** ở `awaiting_review`; Pi kết thúc turn đó ở safe boundary và chờ operator review. Các racer khác vẫn có tool authority và tiếp tục race. Queue trả về racer nguồn bằng opaque session ID; `Wrong` chỉ queue continuation cho racer đó, còn `Confirm` mới gửi candidate có provenance sang flag-router độc lập. Nếu verifier bác candidate đã Confirm, cùng continuation được queue tự động; chỉ verdict chấp nhận mới được `solved` và hủy toàn bộ sibling.
...
| 2026-09-03 | Gia hạn credential Power theo browser local vault để chịu được run dài và Pi restart | Khi run `running`/`paused`, UI refresh broker lease mỗi 3 giây và khi focus/online/tab visible; API chỉ map key request-local tới durable session hiện hữu, không đổi scope/model và không ghi DB/event. Lease cùng identity được gia hạn in-place để không revoke Pi runtime key; lease hết hạn/đổi identity vẫn revoke. Runner chờ 30 giây và defer missing lease cho Power start/steer để browser recovery không terminalize racer. Focused Docker Python, Pi runner, Web, type và Compose gates đều xanh; M-PI-5 vẫn chờ benchmark live kiểm soát. |
| 2026-09-04 | Candidate review chuyển từ run-global sang racer-local hold | Audit run thật cho thấy candidate từ Racer B nhưng transition `run=paused` khiến sibling đi vào failure path. State mới `awaiting_review` giữ đúng racer nguồn, queue trả opaque session provenance, `Wrong`/negative verifier chỉ queue continuation của racer đó; sibling vẫn có tool authority. Backward-compatible read/reject cho run cũ `paused` được giữ. |
| 2026-09-04 | Ghi racer provenance khi candidate đã được verifier chấp nhận | Event xác nhận chỉ giữ label A/B/C và session ID opaque, không lưu candidate. Banner `Verified` và lane source hiển thị đúng racer thắng; sibling dùng state session thực thay vì bị gán `Solved` theo toàn run. |
| 2026-09-04 | Verified solve fence toàn bộ racer rồi export write-up xác định | `Confirm` chỉ solve sau flag-router độc lập; controller request abort cho mọi session. `GET /v1/runs/{id}/writeup` dựng Markdown no-store từ receipt metadata của racer thắng, không tạo provider turn hay lưu raw flag/candidate. |
| 2026-09-04 | Tách Power skill library khỏi challenge và native Pi discovery | `reviewed-skill-packs/manifest.json` là allowlist duy nhất; runner đọc Markdown bounded từ image, redact flag/key-shaped text và ghép vào prompt Power. `noSkills` của Pi vẫn bật, nên `.pi`/`.agents`/`AGENTS.md` không có đường vào session. |
| 2026-09-04 | Hồi phục racer sau lỗi provider transport trong M-PI-5 | Pi bắt cả lỗi SDK ném trực tiếp lẫn assistant error, retry theo exponential backoff+jitter. Hết một burst, nó không gọi `failPower`: job leased hết hạn được reclaim với cùng Power session/JSONL. Auth/quota/model/schema/abort vẫn terminal. Claim SQLite cũng tắt ORM in-memory timestamp evaluation để expired lease reclaim không lỗi aware/naive. |
| 2026-09-04 | Đưa Pi feed và tool receipts vào một terminal stream theo từng racer trong M-PI-5 | Browser sắp xếp chỉ các record đã review theo thời gian: `CTX`, `PI`, `TOOL`, command và output; tự follow khi operator ở đáy, nhưng không giật scroll khi họ xem evidence cũ. Steer composer luôn hiện, hỗ trợ `Ctrl/⌘+Enter`. Browser từ chối thêm raw/partial flag, bearer hoặc credential-shaped text trước render; candidate vẫn chỉ qua queue/reveal rõ ràng. |
| 2026-09-04 | Tinh chỉnh visual Power Operator Desk thành instrument desk | Chỉ thay đổi presentation trong `.power-shell`: hierarchy tối hơn, signal teal duy nhất cho hoạt động, bề mặt console inset và launch card gọn hơn. Không đổi API, state machine, terminal record hay luồng verifier; giao diện vẫn ưu tiên evidence và ba racer thay vì dashboard trang trí. |
